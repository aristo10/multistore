...
export const loadAllAnnouncement = createAsyncThunk("announcement/loadAllAnnouncement", async () => {
  try {
...
// 4 ================== Add Announcement ====================================
export const addAnnouncement = createAsyncThunk("Announcement/addAnnouncement", async (values) => {
  try {
...
export const deleteAnnouncement = createAsyncThunk("Announcement/deleteAnnouncement", async ({id, status}) => {
  try {
    const { data } = await axios({
      method: "patch",
      headers: {
    ...
    return successHandler(data, `Announcement ${status === "true" ? "show" : "hide"} Successfully`);
  } catch (error) {
    return errorHandler(error, true);
  }
});
