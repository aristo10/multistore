...
export const loadAllJobApplicationStatus = createAsyncThunk("JobApplicationStatus/loadAllJobApplicationStatus", async () => {
  try {
...
export const addJobApplicationStatus = createAsyncThunk("JobApplicationStatus/addJobApplicationStatus", async (values) => {
  try {
    ...
    return successHandler(data, "New JobApplicationStatus Added Successfully");
  } catch (error) {
    return errorHandler(error, true);
  }
});
...
export const updateJobApplicationStatus = createAsyncThunk(
  ...
  async ({ id, values }) => {
    try {
      ...
      return successHandler(data, "Job Application Status Updated Successfully");
    } catch (error) {
      return errorHandler(error, true);
    }
  }
...
export const deleteJobApplicationStatus = createAsyncThunk("JobApplicationStatus/deleteJobApplicationStatus", async ({id,status}) => {
  try {
    const { data } = await axios({
      method: "patch",
      headers: {
    ...
    return successHandler(data, `Job Application Status ${status === "true" ? "Show":"Hide"} Successfully`);
  } catch (error) {
    return errorHandler(error, true);
  }
});
...
const jobApplicationStatusSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(loadAllJobApplicationStatusPaginated.fulfilled, (state, action) => {
      state.loading = false;
      state.list = action.payload.data.getAllJobApplicationStatus;
      state.total = action.payload.data.totalJobApplicationStatus;
    });

    builder.addCase(loadAllJobApplicationStatusPaginated.rejected, (state, action) => {
      state.loading = false;
    ...
    builder.addCase(loadSingleJobApplicationStatus.fulfilled, (state, action) => {
      state.loading = false;
      state.jobApplicationStatus = action.payload.data;
    });

    builder.addCase(loadSingleJobApplicationStatus.rejected, (state, action) => {
      state.loading = false;
      state.error = action.payload.message;
    });
...
export const {clearJobApplicationStatus, editJobApplicationStatus} = jobApplicationStatusSlice.actions;
