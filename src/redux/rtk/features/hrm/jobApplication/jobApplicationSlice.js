...
export const loadAllJobApplication = createAsyncThunk("JobApplication/loadAllJobApplication", async () => {
  try {
    const { data } = await axios.get(`job-application?query=all`);
...
export const addJobApplication = createAsyncThunk("JobApplication/addJobApplication", async (values) => {
  try {
...
export const deleteJobApplication = createAsyncThunk("JobApplication/deleteJobApplication", async ({id,status}) => {
  try {
    const { data } = await axios({
      method: "patch",
      headers: {
    ...
    return successHandler(data, `JobApplication ${status === "true" ? "Show":"Hide"} Successfully`);
  } catch (error) {
    return errorHandler(error, true);
  }
});
...
const jobApplicationSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(loadAllJobApplicationPaginated.fulfilled, (state, action) => {
      state.loading = false;
      state.list = action.payload.data.getAllJobApplication;
      state.total = action.payload.data.totalJobApplication;
    });

    builder.addCase(loadAllJobApplicationPaginated.rejected, (state, action) => {
      state.loading = false;
...
export const {clearJobApplication, editJobApplication} = jobApplicationSlice.actions;
