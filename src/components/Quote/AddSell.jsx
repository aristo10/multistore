...
export default function AddSell({ quote }) {
  const { Option } = Select;
  ...
  const totalCalculator = () => {
    ...
    const subTotal =
      productArray?.reduce((subTotal, current) => {
        ...
        const vat = current?.productVat?.percentage || 0;
  ...
  const saleInvoiceProduct = quote?.quoteProduct?.map((quote) => ({
    productId: quote.productId,
    unitMeasurement: quote.unitMeasurement,
    productQuantity: quote.productQuantity,
    productSalePrice: quote.unitPrice,
    productVat: quote.product?.productVat,
  }));
  ...
  return (
    <Form
      ...
      initialValues={{
        ...
        vatId: [],
        saleInvoiceProduct,
        userId: quote?.quoteOwnerId || null,
      }}
    >
      <SellProducts
        form={form}
