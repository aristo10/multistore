...
export default function SendPurchaseInvoice({
    ...
    try {
      ...
      fileList &&
        fileList.forEach((file) => {
          if (file.name === `Purchase invoice - ${invoice.id} .pdf`) {
            formData.append(
              "files[]",
              file.originFileObj,
              `Purchase invoice - ${invoice.id} .pdf`
            );
          } else {
  ...
  useEffect(() => {
    if (purchase?.singlePurchaseInvoice && companyInfo) {
      setFileList((prev) => {
        if (
          prev.some(
            (file) => file.name === `Purchase invoice - ${invoice.id} .pdf`
          )
        )
          return prev;
        return [
  ...
  }, [companyInfo, currencyName, invoice.id, purchase]);
