...
import { addReturnPurchase } from "@/redux/rtk/features/PurchaseReturnList/PurchaseReturnListSlice";
...
const AddReturnPurchase = () => {
  ...
  const submitHandler = async ({ note, paidAmount }) => {
    ...
    const filteredValues = values.returnPurchaseInvoiceProduct.filter(
      (item) => item.productQuantity !== 0 && item.productQuantity !== null
    );
    const newValues = {
      ...values,
      returnPurchaseInvoiceProduct: filteredValues,
    };
    const resp = await addReturnPurchase(newValues);
