  ...
  const columns = [
    {
      ...
      dataIndex: "id",
      key: "id",
      render:(id)=><Link to={`/admin/hrm/leave/${id}`}>{id}</Link>,
      renderCsv:(id)=>id
    },

    {
      ...
      key: "name",
      dataIndex: "user",
      render: ({ firstName, lastName,id }) => <Link to={`/admin/hrm/staff/${id}`}>{firstName + " " + lastName}</Link>,
...

    {
      ...
      render: ({ id }) => [
        {
          label: <ViewBtn title={"View"} path={`/admin/hrm/leave/${id}`} />,
          key: "view",
        },
      ],
      csvOff: true,
