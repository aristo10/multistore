...
import { deleteAward, loadAllAwardPaginated } from '@/redux/rtk/features/hrm/award/awardSlice';
...
export default function GetAllAward() {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "",
      key: "name",
      render:({name,id})=><Link className='hover:text-primary duration-200' to={`/admin/hrm/award/${id}`}>{name}</Link>
    },
    ...
    },
    {
      ...
      render: ({id,status}) => [
        {
          label: <ViewBtn title={"View"} path={`/admin/hrm/award/${id}`} />,
          key: "view",
        },
        {
          label: (
