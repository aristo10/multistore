...
import { clearEmployeeStatusList, deleteEmployeeStatus, loadAllEmployeeStatusPaginated } from "@/redux/rtk/features/hrm/employeeStatus/employeeStatusSlice";
  ...
  const columns = [
    {
      ...
      dataIndex: "id",
      key: "id",
      render: (id) => <Link to={`/admin/hrm/employment-status/${id}`}>{id}</Link>,
      renderCsv: (id) => id,
    },
    {
      id: 2,
      ...
      key: "name",
      render: (name, { id }) => (
        <Link to={`/admin/hrm/employment-status/${id}`}>{name}</Link>
      ),
      renderCsv: (name) => name,
    },
    {
    ...
    },
    {
      ...
      render: ({ id, status }) => [
        {
          label: (
            <ViewBtn title={"View"} path={`/admin/hrm/employment-status/${id}`} />
          ),
          key: "view",
        },
        {
