...
import { clearDesignationList, deleteDesignation, loadAllDesignation } from "@/redux/rtk/features/hrm/designation/designationSlice";
  ...
  const columns = [
    {
      ...
      dataIndex: "id",
      key: "id",
      render: (id) => <Link to={`/admin/hrm/designation/${id}`}>{id}</Link>,
      renderCsv: (id) => id,
    },
    {
      id: 2,
      ...
      key: "name",
      render: (name, { id }) => (
        <Link to={`/admin/hrm/designation/${id}`}>{name}</Link>
      ),
    ...
    },
    {
      ...
      render: ({ id, status, name }) => [
        {
          label: <ViewBtn title={"View"} path={`/admin/hrm/designation/${id}`} />,
          key: "view",
        },
        {
          label: (
            <UserPrivateComponent permission={"update-designation"}>
              <Link
                to={`/admin/hrm/designation/${id}/update`}
