...
import { deleteJobInterview, editJobInterview, loadAllJobInterviewPaginated } from '@/redux/rtk/features/hrm/jobInterview/jobInterviewSlice';
...
export default function GetAllJobInterview() {
   ...
   const {list:jobList, loading:jobLoading}= useSelector(state=>state.job);
...
const columns = [
  ...
  },
  {
      ...
      render: (interviewStatus) => (
          <Tag
             color={ interviewStatus === "PENDING" ?"orange" :"green"}
          >
              {interviewStatus}
          </Tag>
      ),
  },
  {
    ...
    render: (item) => [
      {
          label: <ViewBtn title={"View"} path={`/admin/hrm/job-interview/${item.id}`} />,
          key: "view",
        },
      {
        label: (
