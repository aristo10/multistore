...
import { deleteJob, editJob, loadAllJobPaginated } from '@/redux/rtk/features/hrm/job/jobSlice';
...
export default function GetAllJob() {
   ...
   const {list:jobLocation,loading:jobLocationLoading} = useSelector(state => state.jobLocation);
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "",
      key: "jobTitle",
      render:({id,jobTitle})=><Link className='hover:text-primary' to={`/admin/hrm/job/${id}`}>{jobTitle}</Link>,
...

    {
      ...
      render: (item) => [
        {
            label: <ViewBtn title={"View"} path={`/admin/hrm/job/${item.id}`} />,
            key: "view",
          },
        {
          label: (
