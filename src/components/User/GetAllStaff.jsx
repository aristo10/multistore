...
import { clearUserList, deleteStaff, loadAllStaff } from "@/redux/rtk/features/hrm/user/userSlice";
  ...
  const columns = [
    {
      ...
      dataIndex: "id",
      key: "id",
      render: (id) => <Link to={`/admin/hrm/staff/${id}`}>{id}</Link>,
      renderCsv: (id) => id,
    },
    {
      id: 2,
      ...
      key: "username",
      render: (username, { id }) => (
        <Link to={`/admin/hrm/staff/${id}`}>{username}</Link>
      ),
    },
    {
      id: 3,
    ...
    },
    {
      ...
      render: ({ id, status }) => [
        {
          label: <ViewBtn title={"View"} path={`/admin/hrm/staff/${id}`} />,
          key: "view",
        },
        {
          label: (
  ...
  const filters = [
    {
      ...
      label: "Role",
      type: "select",
      options: list?.map(({ id, username }) => ({ value: id, label: username })),
