...
import { addReturnSale } from "@/redux/rtk/features/SaleReturnList/SaleReturnListSlice";
...
const AddReturnSale = () => {
  ...
  }, [returnSaleInvoice, singleSaleInvoice]);
  ...
  const submitHandler = async ({ note, paidAmount }) => {
    ...
    const filteredValues = values.returnSaleInvoiceProduct.filter(
      (item) => item.productQuantity !== 0 && item.productQuantity !== null
    );
    const newValues = { ...values, returnSaleInvoiceProduct: filteredValues };
    const resp = await addReturnSale(newValues);
  ...
  const updateDamage = useCallback(
    ({ id, value }) => {
      const item = list.find((item) => item.id === id);
      if (item) {
        formData[id] = { ...formData[id], damageQuantity: value };
        item.damageQuantity = value;
        setList([...list]);
        setFormData({ ...formData });
      }
    },
  ...
  const totalReturnQuantity = useCallback(() => {
