...
export default function SendSaleInvoice({
    ...
    try {
      ...
      fileList &&
        fileList.forEach((file) => {
          if (file.name === `Sale invoice - ${invoice.id} .pdf`) {
            formData.append(
              "files[]",
              file.originFileObj,
              `Sale invoice - ${invoice.id} .pdf`
            );
          } else {
  ...
  useEffect(() => {
    if (sale && companyInfo) {
      setFileList((prev) => {
        if (
          prev.some((file) => file.name === `Sale invoice - ${invoice.id} .pdf`)
        )
          return prev;
        ...
        return [
          ...prev,
          {
            name: `Sale invoice - ${invoice.id} .pdf`,
  ...
  }, [sale, companyInfo, currencyName, invoice.id]);
