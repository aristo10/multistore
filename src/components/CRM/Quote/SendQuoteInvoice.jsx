...
import TagInput from "@/components/CommonUi/TagInput";
import quotePDF from "@/utils/quotePDF";

export default function SendQuoteInvoice({
  setSendEmail,
  body,
  invoice,
  subject,
    ...
    try {
      ...
      fileList &&
        fileList.forEach((file) => {
          if (file.name === `Quote invoice - ${invoice.id} .pdf`) {
            formData.append(
              "files[]",
              file.originFileObj,
              `Quote invoice - ${invoice.id} .pdf`
            );
          } else {
  ...
  useEffect(() => {
    if (quote && companyInfo) {
      setFileList((prev) => {
        if (
          prev.some((file) => file.name === `Quote invoice - ${invoice.id} .pdf`)
        )
          return prev;
        ...
        return [
          ...prev,
          {
            name: `Quote invoice - ${invoice.id} .pdf`,
            originFileObj: quotePDF(
              quote,
              companyInfo,
              "Quote Invoice",
              currencyName
  ...
  }, [companyInfo, currencyName, invoice.id, quote]);
