...
const TrialBalance = () => {
  ...
  const {list: store,loading: storeLoading,defaultStore} = useSelector((state) => state.store);

  const handleChange = (value, key) => {
    setPageConfig({ ...pageConfig, [key]: value });
  };
  let storeName = null;
  if (store && pageConfig.storeId) {
    const filteredStore = store.filter((item) =>
      pageConfig.storeId.includes(item.id)
    );
    storeName = filteredStore.map((item) => item.name).join(", ");
  }
