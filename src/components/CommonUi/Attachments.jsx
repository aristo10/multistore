...
import AddAttachment from "../CRM/Attachment/AddAttachment";
	...
	const columns = [
		{
			title: "Name",
			key: "attachmentName",
			render: ({attachmentName, attachmentPath}) =><Link target="_blank" to={window.env.VITE_APP_API+"media/view/"+attachmentPath}>{attachmentName}</Link>,
	...
	
		{
			...
			render: (attachmentOwner) => (
				<Link to={`/admin/hrm/staff/${attachmentOwner?.id}`}>{attachmentOwner?.fullName}</Link>
			),
			renderCsv:(attachmentOwner)=> `${attachmentOwner?.firstName} ${attachmentOwner?.fullName}`
		},
